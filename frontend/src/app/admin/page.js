...
const STATUS_PIE_COLORS = ["#ca8a04", "#2563eb", "#9333ea", "#16a34a"];
...
export default function AdminPage() {
    ...
    const pieCountryData = analytics
        ? [
            {
                name: "Somalia",
                value: analytics.byCountry?.Somalia?.count || 0,
            },
            {
                name: "Ethiopia",
                value: analytics.byCountry?.Ethiopia?.count || 0,
            },
        ]
        : [];

    const pieStatusData = analytics
        ? [
            { name: "Awaiting", value: analytics.byStatus?.CREATED || 0 },
            {
                name: "Verifying",
                value: analytics.byStatus?.AWAITING_FUNDS_CHECK || 0,
            },
            {
                name: "Processing",
                value: analytics.byStatus?.PAID_IN || 0,
            },
            {
                name: "Completed",
    ...
    const barChartData = Array.isArray(chartData)
        ? chartData.map((d) => ({
            date: d.date?.slice(5),
            volume: d.volume ?? 0,
        }))
    ...
    return (
        <div className="min-h-screen bg-stone-50">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
                    <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
                        ...
                        <p className="mt-2 text-sm text-stone-600">
                            Are you sure the funds have reached the recipient?
                            This will mark the transfer as{" "}
            ...
            <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
                ...
                {/* Tabs */}
                <div className="mb-6 flex gap-1 rounded-xl bg-stone-200 p-1 w-fit">
                    {tabs.map((tab) => (
                        <button
                            key={tab.key}
                            onClick={() => setActiveTab(tab.key)}
                            className={`rounded-lg px-6 py-2 text-sm font-semibold transition-colors ${activeTab === tab.key
                        ...
                        <div className="rounded-2xl bg-white p-6 shadow-sm">
                            ...
                            {barChartData.length === 0 ? (
                                <p className="py-8 text-center text-sm text-stone-400">
                                    No completed transactions yet. Generate
                                    demo data to see charts.
                                </p>
                            ) : (
                                <ResponsiveContainer
                                    width="100%"
